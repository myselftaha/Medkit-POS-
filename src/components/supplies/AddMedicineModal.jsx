...
import { X, Save, FileText, Package, DollarSign, Settings } from 'lucide-react';
...
const AddMedicineModal = ({ isOpen, onClose, onSave, suppliers, initialSupplier }) => {
    ...
    const [formData, setFormData] = useState({
        // Basic Info
        name: '',
        formulaCode: '', // Generic Name
        barcode: '',
        category: 'Tablets',
        type: 'Branded',
        supplierName: '',
        manufacturer: '', // New field mapping to description or notes if needed, or just stored

        // Stock & Batch
        batchNumber: '',
        boxNumber: '', // Rack Location
        manufacturingDate: '',
        expiryDate: '',
        stock: '', // Quantity
        unit: 'Strip',
        netContent: '10', // Pack Size
        ...
        cgstPercentage: '0', // GST (Using CGST as a proxy for total tax if needed, or split)
        taxableAmount: '0.00',
        ...
        // Hidden/Calculated/Default
        description: '',
        freeQuantity: '0',
        sellPrice: '', // Synced with price
        itemAmount: '0.00',
        discountAmount: '0.00',
        sgstPercentage: '0',
        igstPercentage: '0',
    ...
    const calculateTotals = (data) => {
        ...
        const gstPerc = parseFloat(data.cgstPercentage) || 0; // Using CGST as total GST input for simplicity if user enters one value
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await onSave({
                ...
                status: formData.status ? 'Active' : 'Inactive' // Map boolean/string switch if needed
            });
            onClose();
        } catch (error) {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
                ...
                {/* Removing the duplicate tab header above - keeping only the bottom one which matches the screenshot style */}
                <style jsx>{`
